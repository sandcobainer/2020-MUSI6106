...
protected:
    ...
    // allocate and free memory of delay line internally
    virtual Error_t initIntern();
    virtual Error_t resetIntern();

    int getNumChannels() const;
    static long int    m_DelayLineLength;

private:
    CCombFilterIf(const CCombFilterIf& that);
    CombFilterType_t m_eFilterType;

    virtual Error_t FIRIntern(float** ppfInputBuffer, float** ppfOutputBuffer, int iNumberOfFrames) = 0;
    virtual Error_t IIRIntern(float** ppfInputBuffer, float** ppfOutputBuffer, int iNumberOfFrames) = 0;
